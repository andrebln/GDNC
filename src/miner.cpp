...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GDNCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GDNCMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GDNCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gdnc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("GDNCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running GDNCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadGDNCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadGDNCMiner() exception");
    }

    LogPrintf("ThreadGDNCMiner exiting\n");
}
