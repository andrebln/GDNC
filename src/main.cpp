...
#if defined(NDEBUG)
#error "GDNC cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GDNC Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	} else if (nHeight > 1 && nHeight <=2) {
		nSubsidy = 31536 * COIN;
	} else if (nHeight > 2 && nHeight <= 3) {
		nSubsidy = 51560 * COIN; 
	} else if (nHeight > 3 && nHeight <= 4) {
		nSubsidy = 21024 * COIN;
	} else if (nHeight > 4 && nHeight <= 5) {
		nSubsidy = 750000 * COIN;
	} else if (nHeight > 6 && nHeight <= 5000) {
		nSubsidy = 0.1 * COIN;
	} else if (nHeight > 5000 && nHeight <= 1051200) {
		nSubsidy = 15 * COIN;
	}
    return nSubsidy;
}
